...
// src/core/node.ts
function createSerovalNode(t, i, s, c, m, p, e, a, f, b, o) {
  return {
    t,
    i,
    s,
...
function deserializeTypedArray(ctx, depth, node) {
  ...
  const result = assignIndexedValue(ctx, node.i, new construct(source, offset));
  return result;
}
function deserializeDataView(ctx, depth, node) {
  ...
  const result = assignIndexedValue(ctx, node.i, new DataView(source, offset));
  return result;
}
...
function serializeTypedArray(ctx, node) {
  return "new " + node.c + "(" + serialize(ctx, node.f) + "," + node.b + ")";
}
function serializeDataView(ctx, node) {
  return "new DataView(" + serialize(ctx, node.f) + "," + node.b + ")";
}
