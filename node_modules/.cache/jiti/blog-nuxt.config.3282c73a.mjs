...
const demosPath = _nodePath.default.join(process.cwd(), 'data', 'demos.json');
const demos = JSON.parse((0, _nodeFs.readFileSync)(demosPath, 'utf8'));
const demoSlugs = new Set(demos.map((demo) => demo.slug));
const legacyDir = _nodePath.default.join(process.cwd(), 'legacy', 'posts');
const legacyFiles = (() => {
  try {
    return (0, _nodeFs.readdirSync)(legacyDir);
  } catch {
    return [];
  }
})();
